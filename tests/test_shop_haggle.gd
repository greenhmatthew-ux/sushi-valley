...
func _shop_shell_fits_the_game_viewport() -> void:
	...
	# Stone Soup used to borrow Mushroom Soup's picture. Every item now owns its art,
	# so the shop shelf shows what it is selling — see tests/test_item_icons.gd.
	var soup_icon := shop._icon_node("stone_soup") as TextureRect
	check_eq("Shop draws each item's own icon",
		soup_icon.texture.resource_path, "res://assets/icons/items/stone_soup.png")
	soup_icon.free()
	shop._close()
